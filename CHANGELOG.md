...
### Added
- **Manage Syncthing** option (`federver` → `14`). Real-time bidirectional folder sync between devices, peer-to-peer, as a standalone Docker container with `--network=host`, `--restart=unless-stopped`, persistent volumes under `/opt/syncthing`, and `STGUIADDRESS=0.0.0.0:8384` so the web UI is reachable from the LAN (default is 127.0.0.1 which is unusable with host networking). Opens firewall 8384/tcp, 22000/tcp+udp, 21027/udp. After the container is up, the step uses `docker exec syncthing syncthing --device-id` to read the server's Device ID and prints it alongside pairing instructions for Fedora (`dnf install syncthing`), Android (F-Droid / Play Store), and iOS (Möbius Sync). Idempotent: re-running when the container is already up re-prints the Device ID and dashboard URL for reference. Tools menu renumbered: Sync files 15→16, Save to pass 16→17. Remote desktop 14→15.
