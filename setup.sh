...
_wg_show_instructions() {
    ...
    case $device_type in
        ...
        2)  # Linux
            ...
            echo -e "     ${BOLD}scp $USER@${endpoint}:${WG_DIR}/${peer_name}.conf /tmp/${peer_name}.conf${NC}"
            ...
            echo ""
            echo -e "  Or scan QR with a GUI WireGuard client:"
            echo ""
            echo "$peer_conf" | qrencode -t UTF8
            echo -e "  ${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            ;;
        3)  # Mac
            ...
            echo ""
            echo -e "  1. Install WireGuard:"
            echo -e "     ${BOLD}brew install wireguard-tools${NC}  (if you have Homebrew)"
            echo -e "     Or install from the ${BOLD}Mac App Store${NC} → search 'WireGuard'"
            echo ""
            echo -e "  2. Copy config from server:"
            echo -e "     ${BOLD}scp $USER@${endpoint}:${WG_DIR}/${peer_name}.conf ~/Downloads/${peer_name}.conf${NC}"
            echo ""
            echo -e "  3a. CLI: ${BOLD}sudo wg-quick up ~/Downloads/${peer_name}.conf${NC}"
            echo -e "  3b. GUI: Open WireGuard app → ${BOLD}Import Tunnel(s) from File${NC} → select the .conf"
            echo ""
            echo -e "  Disconnect: ${BOLD}sudo wg-quick down ${peer_name}${NC} (CLI) or toggle off in app"
            echo ""
            echo -e "  Or scan QR:"
            echo ""
            echo "$peer_conf" | qrencode -t UTF8
...
PersistentKeepalive = 25"
    ...
    echo ""
    echo -e "  ${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${YELLOW}CLIENT QUICK REFERENCE${NC}"
    echo ""
    echo -e "  ${BOLD}Phone:${NC}"
    echo -e "    Install WireGuard app → scan QR code shown above"
    echo ""
    echo -e "  ${BOLD}Linux laptop:${NC}"
    echo -e "    ${BOLD}sudo dnf install wireguard-tools${NC}"
    echo -e "    ${BOLD}scp $USER@${endpoint}:${WG_DIR}/<peer-name>.conf /tmp/wg.conf${NC}"
    echo -e "    ${BOLD}sudo cp /tmp/wg.conf /etc/wireguard/wg0.conf${NC}"
    echo -e "    Connect: ${BOLD}sudo wg-quick up wg0${NC}"
    echo -e "    Disconnect: ${BOLD}sudo wg-quick down wg0${NC}"
    echo ""
    echo -e "  ${BOLD}Mac:${NC}"
    echo -e "    ${BOLD}brew install wireguard-tools${NC} or App Store → WireGuard"
    echo -e "    ${BOLD}scp $USER@${endpoint}:${WG_DIR}/<peer-name>.conf ~/Downloads/${NC}"
    echo -e "    Connect: ${BOLD}sudo wg-quick up ~/Downloads/<peer-name>.conf${NC}"
    echo -e "  ${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
}
