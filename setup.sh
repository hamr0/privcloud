...
_wg_show_instructions() {
    ...
    case $device_type in
        ...
        2)  # Linux
            ...
            echo ""
            echo -e "  ${BOLD}Step 1:${NC} Install fedvpn on your laptop (one-time):"
            echo -e "    ${BOLD}cd ~/PycharmProjects/privcloud && git pull${NC}"
            echo -e "    ${BOLD}sudo cp fedvpn /usr/local/bin/fedvpn${NC}"
            echo ""
            echo -e "  ${BOLD}Step 2:${NC} Copy the config below (or later from laptop with):"
            echo -e "    ${BOLD}ssh -t $USER@${endpoint} \"sudo cat ${WG_DIR}/${peer_name}.conf\"${NC}"
            echo ""
            echo -e "  ${DIM}───────────────────────────────────────${NC}"
            echo "$peer_conf"
            echo -e "  ${DIM}───────────────────────────────────────${NC}"
            echo ""
            echo -e "  ${BOLD}Step 3:${NC} On your laptop, run:"
            echo -e "    ${BOLD}fedvpn${NC} → ${BOLD}1 (setup)${NC} → paste the config → ${BOLD}Ctrl+D${NC}"
            echo ""
            echo -e "  ${BOLD}Step 4:${NC} Connect/disconnect:"
            echo -e "    ${BOLD}fedvpn${NC} → ${BOLD}2${NC} connect  |  ${BOLD}fedvpn${NC} → ${BOLD}3${NC} disconnect"
            echo -e "    Or: ${BOLD}fedvpn start${NC}  |  ${BOLD}fedvpn stop${NC}"
            echo -e "  ${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            ;;
        3)  # Mac
            echo -e "  ${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
PersistentKeepalive = 25"
    ...
    echo ""
    echo -e "  ${BOLD}Phone:${NC} Install WireGuard app → scan QR code shown above"
    echo ""
    echo -e "  ${BOLD}Linux:${NC} Run ${BOLD}fedvpn${NC} → 1 (setup) → paste the config shown above"
    echo -e "         Then: ${BOLD}fedvpn${NC} → 2 (connect) / 3 (disconnect)"
    echo ""
    echo -e "  ${BOLD}Mac:${NC} Install WireGuard (brew or App Store) → save config to file → import"
    echo -e "  ${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
}
