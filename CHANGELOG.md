...
- **AdGuard install (`federver` → 12)** three big UX fixes:
  - **No more port-3000 wizard detour.** The install now asks for the admin username + password up front, hashes it with `htpasswd -B` (bcrypt), and pre-seeds `/opt/adguard/conf/AdGuardHome.yaml` with all defaults (listen on 0.0.0.0:53 / 0.0.0.0:80, Cloudflare + Quad9 DoH upstreams, AdGuard DNS filter + AdAway blocklists enabled). Container boots directly on port 80 with a working config. Zero post-install click-through. `httpd-tools` is installed automatically if `htpasswd` isn't present.
